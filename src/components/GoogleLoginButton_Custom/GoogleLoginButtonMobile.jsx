...
const GOOGLE_WEB_CLIENT_ID =
  (typeof import.meta !== "undefined" && import.meta.env && (import.meta.env.VITE_GOOGLE_CLIENT_ID || import.meta.env.VITE_GOOGLE_CLIENT_ID_WEB))
    ? (import.meta.env.VITE_GOOGLE_CLIENT_ID || import.meta.env.VITE_GOOGLE_CLIENT_ID_WEB)
    : "";
...
const obtenerTipoYPerfil = (propTipo, propPerfil) => {
  ...
  let p = propPerfil;
  try {
    if (!t) t = localStorage.getItem("tipoCuentaRegistro") || localStorage.getItem("tipoCuentaIntentada") || null;
    if (!p) {
      const crudo = localStorage.getItem("perfilCuentaRegistro") || localStorage.getItem("perfilCuentaIntentada") || null;
      if (crudo) { try { p = JSON.parse(crudo); } catch { p = { perfil: crudo }; } }
    }
  } catch {}
...
const GoogleLoginButtonMobile = ({ onClose, onRegistroExitoso, modo = "login", tipo, perfil }) => {
  const { iniciarSesion } = useContext(AuthContext);
  const [busy, setBusy] = useState(false);
  const [forceWeb, setForceWeb] = useState(false);
  const nonce = useMemo(() => genNonce(), []);

  // Inicializa el plugin con el WEB CLIENT ID (requerido en Android)
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.warn("[Google init] fallo SocialLogin.initialize:", e);
      }
    })();
  }, []);
  ...
  const handleNativeGoogle = async () => {
    ...
    try {
      ...
      const res = await SocialLogin.login({
        provider: "google",
        options: { scopes: ["profile", "email"], nonce },
      });
      ...
      if (modo === "link") {
        ...
        showWarning("Google", r?.data?.mensaje || "No se pudo vincular Google.");
        return;
      }
      ...
      showWarning("Google", r?.data?.mensaje || "No se pudo autenticar con Google.");
    } catch (e) {
      setForceWeb(true);
      showInfo("Google Login", (e?.message || "Fallo inicio nativo") + " — usando método web…");
  ...
  const WebButton = (
    <button
      type="button"
      onClick={() => showInfo("Botón desactivado", "El login web con Google fue deshabilitado para evitar requests externos.")}
      style={{ height: 40, borderRadius: 12, border: "1px solid #e5e7eb", background: "#f8fafc", color: "#64748b", fontSize: 15, display: "inline-flex", alignItems: "center", justifyContent: "center", width: "100%" }}
    >
  ...
  const NativeButton = (
    <button
      ...
      style={{ height: 40, borderRadius: 12, border: "1px solid #e5e7eb", background: "#ffffff", color: "#111827", fontSize: 15, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 8, width: "100%" }}
    >
      ...
      {modo === "link" ? "Vincular con Google" : "Continuar con Google"}
    </button>
  );
  ...
  return (
    <div style={{ width: "100%", display: "grid", position: "relative" }}>
      {busy && (
        <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", background: "rgba(255,255,255,0.6)", backdropFilter: "blur(2px)", borderRadius: 12, zIndex: 2 }}>
          <div style={{ width: 22, height: 22, border: "3px solid #cbd5e1", borderTopColor: "#1745CF", borderRadius: "50%", animation: "gspin 800ms linear infinite" }} />
