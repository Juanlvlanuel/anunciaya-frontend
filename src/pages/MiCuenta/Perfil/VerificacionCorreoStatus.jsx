...
export default function VerificacionCorreoStatus({ verificado = false, onReenviar }) {
  ...
  const resend = async () => {
    ...
    try {
      ...
      setErr(e?.message || "No se pudo enviar el correo de verificación.");
    } finally {
      setLoading(false);
    }
  };
      ...
      {verificado ? (
        ...
        <div className="space-y-2">
          ...
          <button
            onClick={resend}
            disabled={loading}
            className="text-sm px-3 py-2 rounded-xl border hover:bg-gray-50 dark:hover:bg-zinc-800 disabled:opacity-60 disabled:cursor-not-allowed"
          >
