  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!cancelled) setError(e?.message || "No se pudieron cargar los tutoriales.");
      } finally {
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {list.map((t) => (
            <div key={t.id || t._id} className="rounded-lg border border-gray-200 dark:border-zinc-800 p-3">
              ...
              <button
                className="mt-2 text-xs px-3 py-1.5 rounded-xl border hover:bg-gray-50 dark:hover:bg-zinc-800"
                onClick={() => open(t)}
              >
                Ver
              </button>
